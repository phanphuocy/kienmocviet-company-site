...
const ProjectPage = ({ children, data }) => {
    ...
    const hero = data.thumbnailsJson.src.childImageSharp.fluid
    console.log(prop)

    const index = useRef(0);
    const [springs, set] = useSprings(prop.sections.length, i => ({ x: i * 768, display: 'block' }));
    ...
    const transform = () => {
        ...
        set(i => {
            ...
            if (i < index.current - 1 || i > index.current + 1) return { x, display: 'none' }
            ...
            >
                ...
                <WidthConstraint>
                    ...
                    <p>
                        ...
                        <button style={{ width: "30px", marginRight: "10px" }} onClick={() => { incrementIndex() }} >+</button>
                        <button style={{ width: "30px" }} onClick={() => { decrementIndex() }} >-</button>
                    </p>
                    <div className="asacontainer">
                        {springs.map(({ x, display }, i) => (
                            <animated.div {...transform()} key={i} style={{ display, transform: x.interpolate(x => `translate3d(${x}px,0,0)`) }} >
                                <div style={{ width: "700px", height: "100px", backgroundColor: "#eee" }}></div>
                            </animated.div>
                        ))}

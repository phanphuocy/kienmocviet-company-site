...
const CarouselWithPreview = ({ thumbnails, images }) => {
  ...
  const switchToSelector = selectorSlug => {
    ...
    let foundIndex = transformedData.findIndex(
      item => item.tag === selectorSlug
    );
    if (foundIndex >= 0) {
  ...
  return (
    <div {...bind}>
      <div className={styles.tagsWrap}>
        {tagSelectors.map(button => (
          <button
            key={button.slug}
            onClick={() => switchToSelector(button.slug)}
          >
            {button.name}
          </button>
        ))}
        ...
        <div className={styles.preview}>
          <div className={styles.fill}>
            {images[current].source.sharp.fluid ? (
              <Img
                fluid={images[current].source.sharp.fluid}
        ...
        <div className={styles.fill}>
          <div style={{ position: "relative" }}>
            {transitions.map(
              ({
                item,
                props: { x, display, height, width, ...rest },
                key
              }) => (
                <animated.div
                  key={key}
                  onClick={() => switchByKey(key)}
                  style={{
                    height: height,
                    width: width,
                    position: "absolute",
                ...
                >
                  <div className={styles.card} style={{ display: display }}>
